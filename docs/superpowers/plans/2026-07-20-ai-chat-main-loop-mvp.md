...
> **2026-07-22 节奏修订（优先于下方旧任务描述）：** PK 后人物任务保持 `locked`。炎鑫首次私信只承接 PK，不得提前提十秒剪辑或取证；玩家与 AI 完成两次成功的普通往返后，推荐流解锁可实际观看的 `E103`；玩家看完 `E103` 后任务才进入 `invited`，随后由 `clip_followup` 询问玩家看法并进入取证对话。模型失败只显示系统提示，不自动推进；连续失败两次后由玩家显式点击离线保障继续。

## Global Constraints

- 本计划只实现一名人物“炎鑫”、一个人物任务、一组 PK 是/否节点、一条关系派生视频和一个 AI 来信结局。
- 纵切片固定因果链为 `W001 + 梯子 → W101 + 空调师傅 → W300 与 E001 → PK 首信与两次普通往返 → E103 十秒争议视频 → 私聊取证任务 → E201 小黄车录音笔 → C001 + 录音笔 → C101 小黄车投影服务，同时 W300 + 录音笔 → W301 → W301 + 投影服务 → W400 → AI 来信`。`C001` 与 `W300` 的两次录音笔投放顺序可以互换。
...
- 视频资产处理必须遵循 `刷到你了/14_MVP视频资产变更与制作清单_V0.1.md`：14 条旧视频复用，`K101` 改版，五个 E 节点新增。
...
- 聊天不是关键词解谜。两次成功的普通往返只负责建立熟悉感并解锁 `E103`；任务阶段只能由玩家看过 `E103` 后的有效语义证据推进。
- AI 服务失败、达到 8 秒超时、限流或结构化输出校验失败后只显示系统提示，不得伪装成角色回复，也不得自动推进任务；连续失败两次后提供玩家可显式选择的离线保障。
...
3. 玩家点进 `E001`，看到 PK 最后 30 秒，选择“上票”或“不上票”；炎鑫的首条主动私信经过轻微延迟到达，只聊刚才的 PK，不显示人物状态或倒计时，也不提前讲剪辑任务。
4. 玩家与炎鑫完成两次成功的普通往返后，推荐流出现 `E103`；玩家亲自看完这条只有最后十秒、缺少上下文的争议切片。
5. 炎鑫随后主动询问玩家怎么看；玩家识别断章取义并接受完整取证方案后，炎鑫主动报备设备测试进展；报备送达时解锁 `E201`，视频说明自己从开播前录到下播后并挂载录音笔。
...
8. 关闭 AI 服务重复上述流程，连续失败两次后由玩家点击离线保障继续；没有任何系统消息自动替玩家做决定。
...
| 编号 | 测试内容 | 必须断言 | 阻断 |
| --- | --- | --- | --- |
| A-01 | 内容目录完整性 | `E001/E101/E102/E103/E201` ID 唯一；关系节点商品只来自预定义目录；所有节点可以通过校验 | 是 |
| A-02 | 录音笔来源 | `K101` 无商品；`E201` 是录音笔唯一首次来源；录音笔可重复购买且单价固定 30 | 是 |
| A-03 | 主线物品用途 | 第一支录音笔可用于 `C001 → C101`；第二支可用于 `W300 → W301`；两种先后顺序结果一致 | 是 |
| A-04 | PK 上票 | 扣除 30，一次性写入支持和边界压力证据，任务保持 `locked` 并触发首次私信；重复提交不再扣款 | 是 |
| A-05 | PK 不上票 | 扣除 0，写入尊重证据，任务保持 `locked` 并触发首次私信；不被记录为错误选择 | 是 |
| A-06 | 熟悉与任务门槛 | 两次成功普通往返只解锁 `E103`；系统失败不计入；看完 `E103` 才进入 `invited`；任意模型文本不能直接解锁节点 | 是 |
...
**验证目标：** 不接真实 AI，通过 fake client 和玩家显式离线保障完整体验 PK、私聊、十秒争议视频、轻微回复延迟、主动报备、关系视频解锁和两次购买，并确认玩家看不到后台状态与暗数值。
...
| B-03 | 首次主动私信 | 两条 PK 路线都会收到炎鑫主动消息；只承接 PK，不提十秒剪辑、录像、证据或取证；消息不是选择提交后同一渲染帧出现 | 是 |
...
| B-07 | 视频门槛与报备 | 两次成功往返后 `E103` 出现在推荐流；看完后才触发 `clip_followup`；设备测试报备到达前 `E201` 不可见，到达后只解锁一次 | 是 |
| B-08 | 无可见状态 | 聊天页、推荐流和个人页均不存在在线、直播中、办事中、休息中、输入中、倒计时和关系数值 | 是 |
| B-09 | AI 失败降级 | 8 秒超时、HTTP 503、非法 JSON 和未知信号只显示系统提示；不自动推进；连续失败后玩家可显式选择离线保障 | 是 |
...
**失败后禁止继续的情况：** 玩家把不上票理解为失败、首信提前讲剪辑任务、没有实际 `E103`、消息只能停留在聊天页才会到达、刷新造成重复消息、状态/倒计时暴露、系统失败自动替玩家推进、两次购买被合并。
...
| C-06 | 角色与阶段差异 | support 与 hold_back 首信反映不同 PK 经历且都不提前讲任务；只有 `clip_followup` 可承接已观看的十秒剪辑 | 是 |
| C-07 | 服务超时 | provider 超时返回 503；前端显示非角色系统提示，仍经过软时间投递而非突然同步出现 | 是 |
| C-08 | 限流 | 同一 IP 每分钟第 21 个请求返回 429；前端显示系统提示，不丢失玩家消息 | 是 |
| C-09 | 日志隐私 | 日志只有 request id、延迟、状态和降级原因；不含正文、历史、模型完整输出或 API key | 是 |
| C-10 | 密钥隔离 | API key 只从服务端环境变量读取；前端源码、网络响应和构建产物均不存在密钥 | 是 |
| C-11 | AI 离线 | 完全停止 `ai-server` 后不会自动推进；玩家显式选择离线保障后，两条关系路线仍可到达 `E201` | 是 |
