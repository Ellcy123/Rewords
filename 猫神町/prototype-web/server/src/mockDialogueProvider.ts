...
const responses: Record<string, { line: string; player: string; lead?: string; emotion: string; action: string }> = {
  ask_memory: { line: "看这里，刻着真昼的名字。你别光看我，看看它。", player: "看到了。", lead: "纱夜开的门，九条给的票，后殿钥匙也在我这里。总有一处能查下去。", emotion: "急切", action: "她把刻着“真昼”的猫铃按进你掌心。" },
  ...
  saya_open_gentle: { line: "看吧。编号和名字都对得上，怎么回来的，我不知道。", player: "嗯，我看清了。", lead: "这张通行令上有九条家的印章。当晚让我开门的，就是这份命令。", emotion: "僵硬", action: "她亮出胸牌夹层里的通行令复写纸。" },
  saya_open_press: { line: "我收到了通行令。可门是我开的，这件事推不掉。", player: "我在听。", lead: "你看这个印章。九条家的。", emotion: "恼火", action: "她从胸牌夹层抽出通行令复写纸。" },
  saya_open_absurd: { line: "它要是会自己走，我还能跟上去看看。可我不知道是谁放回来的。", player: "这确实怪。", lead: "这份通行令倒一直在我这里。印章你认得吗？", emotion: "冷淡", action: "她亮出盖着九条家印章的复写纸。" },
