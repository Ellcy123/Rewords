...
export const HEART_PROMPT_VERSION = "shixu-v18-information-pressure";
...
export function buildHeartPrompt(c: HeartContext, baseUser: string) {
  ...
  const system = [
    ...
    "拾绪不是把对话变成情绪疗愈，而是用玩家选的态度改变人物此刻的决定。每次出牌后，NPC必须因这种态度在信息、条件、要求、关系边界或实际行动中选择一个具体方向。不得只说感谢、理解、心里好受了一点，然后继续原话题。",
    "优先使用undisclosed_to_player中与当下有关的1至2个事实制造冲击：直说、部分承认、否认、提出交换或拿真信息包住回避。没有可说的新事实时，根据role.dramaticMoves产生具体质问、交易、决定、威胁、约见或指向下一人的行动。一段只要一个主冲击，但必须让玩家知道‘刚才发生了什么变化’。",
    ...
    'disclosed_fact_ids只填本段NPC台词明确告诉遥的事实ID，仅在NPC内心或说谎逻辑中用到的私密事实不得填入。progress如实摘要本段的具体推进，不能写深化情绪、建立信任或继续交流。格式：{"beats":[{"speaker":"npc或player","line":"台词","stage_direction":"","emotion":"情绪"}],"can_continue":true,"choice_point":null,"action_plan":null,"consequence":null,"closing_reason":"未结束时为空","used_fact_ids":[],"disclosed_fact_ids":[],"progress":{"type":"reveal|contradiction|request|deal|decision|threat|action|transition","summary":"具体发生的变化"},"pickup":null}。需要玩家表态时choice_point={"quote":"末句NPC台词摘录","reason":"为什么需要玩家选择立场"}。产牌时pickup={"beat_index":0,"kind":"fear|sympathy|affection|sadness|anger|joy|curiosity|doubt|contempt之一","quote":"原文摘录"}。提交前数beats项数：只能1至5项，还受maxGeneratedLines限制；未结束也只写下一段。每次必须包含consequence字段，但允许为null；有自然、合法后果时尽量填写，不能为避免写清推进而惯性填null。每次必须包含action_plan：若NPC本段明确答应地点时间或发出具体赴约邀请，必须填上述会面对象，不能只在台词里答应却遗漏计划。收尾can_continue=false也能同时有action_plan，约定发生在离开之前。'
  ].join("\n");
...
export function validateHeartDraft(raw: unknown, c: HeartContext, known: string[]): HeartDraftData {
  ...
  if (d.disclosed_fact_ids.some(id => !d.used_fact_ids.includes(id) || !known.includes(id))) throw new Error("unknown_fact");
  if (d.progress.type === "reveal" && !d.disclosed_fact_ids.length) throw new Error("off_topic");
  if (d.progress.type === "action" && !d.consequence) throw new Error("invalid_consequence");
  if (/^(?:深化情绪|建立信任|继续交流|回应对方|安慰对方)$/.test(d.progress.summary)) throw new Error("off_topic");
  ...
  if (d.consequence && ["material", "case_action"].includes(d.consequence.type) && d.beats[d.consequence.beatIndex].stage_direction.trim()) throw new Error("invalid_consequence");
  if (d.pickup) {
...
export function heartResult(d: HeartDraftData, c: HeartContext, provider: "deepseek" | "mock"): DialogueResult {
  ...
  return DialogueResultSchema.parse({ ...beats[0], continuations: beats.slice(1), options: [],
    ...
    debug: { provider, decision: "拾绪：态度改变事件走向", usedFacts: d.used_fact_ids, disclosedFacts: d.disclosed_fact_ids, sceneGoal: `${d.progress.type}：${d.progress.summary}`, promptVersion: HEART_PROMPT_VERSION, npcActionId: "none" }
  });
}
...
export function mockHeartDialogue(c: HeartContext): DialogueResult {
  ...
  return heartResult({ consequence, action_plan: null, beats, can_continue: !pacing.mustClose && !paused, choice_point: choice ? { quote: "你会不会觉得这样的我很没用？", reason: "小春在意遥对自己的看法，需要玩家表态" } : null,
    closing_reason: pacing.mustClose || paused ? "需要缓一缓" : "", used_fact_ids: [], disclosed_fact_ids: [],
...
function mockTownHeartDialogue(c: HeartContext): DialogueResult {
  ...
  const choice = !closes && c.heartIntent !== "listen";
  ...
  return heartResult({ beats, can_continue: !closes, choice_point: choice ? { quote: row.question, reason: "对方等待遥选择回应态度" } : null,
    closing_reason: closes ? "结束当前交谈" : "", used_fact_ids: [], disclosed_fact_ids: [],
