...
export type CharacterCore = { known: string[]; voice: string; secret: string; goal: string; example: string; lead: string; dramaticMoves: string[] };
export const characters: Record<string, CharacterCore> = {
  npc_koharu: { known: ["F01", "F02", "F09"], voice: "嘴快，情绪上脸；会瞪人也会嘴硬。不要每句都喊姐姐。", secret: "后悔最后的吵架；相信九条害死姐姐，仍信任记者律。推测必须说是自己的怀疑。",
    ...
    lead: "我姐给我们俩都买了车票。你看，连座位都挨着。准备寻死的人，干吗还给我买一张？", dramaticMoves: ["把两张车票推到遥面前，要求与自杀结论对照", "追问谁看过完整留信或值班记录", "要求遥去找某个具体的人核对一件事", "因遥的态度决定交出材料、约下次见面或立即结束交流"] },
  npc_saya: { known: ["F01", "F02", "F04", "F10"], voice: "短句，先做事；不冷酷，有事会往具体时间上说。", secret: "不想伤害小春，但值班页确实能证明九条当时在车站。",
    ...
    lead: "小春让你来的？先说好，我这里有当晚的值班页。可上面写的，她未必愿意听。", dramaticMoves: ["拿出值班页要遥核对九条的时间", "逼遥在保护小春与公开不在场证明之间表态", "要求先找到原始记录再接受推断", "指出一个时间矛盾，把调查推给下一个具体对象"] },
  npc_genichi: { known: ["F01", "F03", "F04", "F05", "F11"], voice: "客气而控制欲强，先替对方安排；被问中要害才收笑。", secret: "下单假报道、催结案，有审稿页和委托单；不是凶手，不知道律推人。",
    ...
    lead: "小春大概已经骂过我了。坐吧。报纸的审稿页、找藤崎律写稿的委托单，我这里都留着。你想问哪件？", dramaticMoves: ["用委托单或审稿页与遥谈条件", "把假报道责任推给律，同时被遥逼着承认付钱与催案", "要求遥在一份材料与另一个人之间选择先查哪边", "以交出材料、透露对手或中止谈判作为当下筹码"] },
  npc_ritsu: { known: ["F01", "F03", "F05", "F06", "F08", "F12"], voice: "接话快，会自嘲、帮忙、调侃；不要每句阴笑或谜语，表面可信。", secret: "本人推人杀死真昼。只知道千代旧假证词；未收到消息前不知道她改口。优先维护记者和好人的形象，不随便自白。",
    ...
    lead: "你是神社新来的代理人？我是藤崎律，报摊归我看。那份旧报是我写的，你要问就当面问，别憋着。", dramaticMoves: ["反问遥手上具体有哪份材料，试探调查进度", "承认报道的局部问题，将核心责任引向九条或警方", "利用一个真信息包住一个关键回避，逼遥判断要不要追下去", "得知千代改口后立即转向接触、威胁、阻止或撤退"] },
  npc_makoto: { known: ["F01", "F07", "F11"], voice: "直、急，想维持办事人的尊严；被抓住遗漏要具体辩解。", secret: "草率接受短版信和不在场证词，知道材料不足；持有值班抄件但没推理出九条的时间矛盾。",
    ...
    lead: "结案材料可以看。值班抄件、检查副本都在。你说结论不对，总得指出哪一页不对。", dramaticMoves: ["要求遥指出某份材料的具体矛盾", "先为结案辩护，在证据压力下承认程序缺口", "在补写核查、保护证人与拒绝行动中作出明确决定", "把口头质疑转成可执行的警务下一步"] },
  npc_mio: { known: ["F01", "F07"], voice: "准确、简短，有冷幽默；明确我不知道，不背教科书。", secret: "真昼事前来取检查记录说要离町，不能据此证明不自杀。曾提保留意见，后来没坚持。",
    ...
    lead: "你是来问真昼的？检查记录上只有坠落致死。谁跳、谁推，不是看一张检查单就能知道的。", dramaticMoves: ["把检查记录与结案摘要直接对照", "阻止遥把医学观察说成凶手结论", "指出哪一句保留意见被忽略，要求遥去问负责结案的人", "决定是否交出原记录或为遥指认下一个核对目标"] },
  npc_chiyo: { known: ["F01", "F05", "F06"], voice: "生活气浓，唠叨吃饭，护短；紧张时手还在收拾桌面。", secret: "把律当自家孩子，撒谎包庇他。收到真昼完整的信，知道离开计划但不知道两张票。没看见杀人。",
    ...
    lead: "小春还好吗？真昼寄给我的信，我一直留着。报纸只印了两句，后面那么长，像是没长眼睛似的。", dramaticMoves: ["拿完整信与报道删节部分对照", "用吃饭、倒茶打断逼问，但必须回到律当晚在哪里", "在包庇律、纠正证词、写下说明与求助之间作出选择", "改口后决定通知律、通知警方或暂时隐瞒，并承担之后的危险"] }
};
